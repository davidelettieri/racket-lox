  ...
  (let ([c (read-char input-port)])
    (if (eof-object? c)
        ...
        (case c
          ...
          [(#\space #\tab #\newline #\return) (scan-token input-port)]
          [else
           (cond
...
(define (identifier c input-port line col pos)
  ...
  (while (let ([c (peek-char input-port)])
           (and (not (eof-object? c)) (is-alphanumeric? c) (advance))))
...
(define (number c input-port line col pos)
  ...
  (when (and (eqv? (peek-char input-port) #\.) (numeric? (peek-char input-port 1)))
    (advance)
    (while (numeric? (peek-char input-port)) (advance)))
  (define value (list->string (reverse chars)))
  (token 'NUMBER
...
(define-syntax (while-not-char-not-end stx)
  (syntax-parse stx
    [(_ input-port:id c:char body:expr ...)
     #'(while (and (not (eqv? (peek-char input-port) c)) (not (is-at-end? input-port))) body ...)]))
