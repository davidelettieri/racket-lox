...
(define (parse tokens)
    ...
    (define (assignment)
        ...
        expression)
    (define (or-syntax)
        (define expr (and-syntax))
        (while (match 'OR)
            (define op (previous))
            (define and (and-syntax))
            (set! expr (datum->syntax #f `(lox-logical ,expr ,op ,and))))
        expr)
    (define (and-syntax)
        (define expr (equality))
        (while (match 'OR)
            (define op (previous))
            (define and (equality))
            (set! expr (datum->syntax #f `(lox-logical ,expr ,op ,and))))
    ...
    (define (equality)
        ...
        (while (match 'BANG_EQUAL 'EQUAL_EQUAL)
            ...
            (set! expr (datum->syntax #f `(lox-binary ,expr ,op ,right))))
        expr)
    (define (comparison)
        (define expr (term))
